...
class EmailTemplateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = EmailTemplate
        fields = [
            'id', 'slug', 'name', 'subject', 'html_content', 'text_content',
            'category', 'is_active', 'metadata', 'has_html_content', 
            'has_text_content', 'created_at', 'updated_at', 'created_by', 'updated_by'
        ]
        read_only_fields = ['id', 'created_at', 'updated_at', 'created_by', 'updated_by']
        extra_kwargs = {
            'slug': {'help_text': 'Unique identifier for the template within organization'},
            ...
            'text_content': {'help_text': 'Plain text email body with template variables'},
            'category': {'help_text': 'Template category for organization'},
            'is_active': {'help_text': 'Whether this template is active and can be used'},
...
class EmailTemplateCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = EmailTemplate
        fields = [
            'slug', 'name', 'subject', 'html_content', 'text_content',
...
class EmailLogSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'recipient_email', 'recipient_user', 'recipient_name',
            'subject', 'status', 'sent_at', 'delivered_at',
            'error_message', 'retry_count', 'provider_message_id',
            'is_pending', 'is_sent', 'is_failed', 'can_retry',
            'created_at', 'updated_at'
        ]
        read_only_fields = [
            'id', 'template', 'template_slug', 'recipient_email', 
            'recipient_user', 'subject', 'status', 'sent_at', 
...
class EmailLogDetailSerializer(EmailLogSerializer):
    ...
    class Meta(EmailLogSerializer.Meta):
        fields = EmailLogSerializer.Meta.fields + [
            'html_content', 'text_content', 'context_data', 'metadata'
        ]
...
class SendEmailSerializer(serializers.Serializer):
    ...
    recipient_user_id = serializers.UUIDField(
        required=False,
        help_text="UUID of user to send email to"
    )
    ...
    send_async = serializers.BooleanField(
        required=False,
        default=True,
        help_text="Whether to send email in background"
    )
...
class EmailTemplatePreviewSerializer(serializers.Serializer):
    ...
    context = serializers.DictField(
        required=False,
        default=dict,
        help_text="Template context variables for preview"
    )
