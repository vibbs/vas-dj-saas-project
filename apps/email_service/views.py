...
class EmailTemplateViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = super().get_queryset().filter(organization=self.request.user.organization)
...
class SendEmailViewSet(viewsets.GenericViewSet):
    ...
    def create(self, request):
        ...
        try:
            ...
            return Response({
                "message": "Email queued for sending" if serializer.validated_data.get('send_async', True) else "Email sent",
                "email_log_id": str(email_log.id)
            })
    ...
    def preview(self, request):
        ...
        try:
            if template_slug:
                ...
                rendered_content = {
                    'subject': Template(subject).render(**template_context) if subject else '',
                    'html_content': Template(html_content).render(**template_context) if html_content else '',
                    'text_content': Template(text_content).render(**template_context) if text_content else '',
                }
