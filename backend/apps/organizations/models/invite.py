...
class Invite(models.Model):
    ...
    token = models.CharField(
        max_length=255, unique=True, help_text="Unique token for accepting the invite"
    )
    ...
    message = models.TextField(
        blank=True, help_text="Optional personal message from the inviter"
    )
    ...
    def clean(self):
        ...
        if self.expires_at is None:
            raise ValidationError({"expires_at": _("Expiry date is required.")})

        # Validate role
        if self.role not in dict(self.ROLE_CHOICES):
            raise ValidationError({"role": _("Invalid role specified.")})
    ...
    def accept(self, user):
        ...
        if user.email.lower() != self.email.lower():
            raise ValidationError(_("Invite email does not match your account email."))
    ...
    def resend(self):
        ...
        if self.is_expired():
            ...
            self.save(update_fields=["status", "updated_at"])
            raise ValidationError(
                _("Cannot resend expired invite. Create a new one instead.")
            )
