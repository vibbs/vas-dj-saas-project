...
class TestOrganizationEdgeCases:
    ...
    def test_organization_plan_trial_inconsistencies(self):
        ...
        org = OrganizationFactory.build(plan="pro", on_trial=True)
        with pytest.raises(
            ValidationError, match="Organizations on trial must have 'free_trial' plan"
        ):
            org.clean()

        # Has paid_until but on trial
        org = OrganizationFactory.build(on_trial=True, paid_until=timezone.now().date())
        with pytest.raises(
            ValidationError,
            match="Organizations on trial cannot have a paid_until date",
        ):
            org.clean()
...
class TestMembershipEdgeCases:
    ...
    def test_membership_role_status_combinations(self):
        ...
        suspended_owner = OrganizationMembershipFactory(
            organization=org, role="owner", status="suspended"
        )
        ...
        invited_admin = OrganizationMembershipFactory(
            organization=org, role="admin", status="invited"
        )
    ...
    def test_membership_extreme_join_dates(self):
        ...
        future_membership = OrganizationMembershipFactory(
            organization=org, joined_at=timezone.now() + timedelta(days=1)
        )
...
class TestInviteEdgeCases:
    ...
    def test_invite_token_collision(self):
        ...
        with patch(
            "apps.organizations.models.invite.secrets.token_urlsafe"
        ) as mock_token:
    ...
    def test_invite_status_date_inconsistencies(self):
        ...
        invite = InviteFactory(
            status="pending", accepted_at=timezone.now(), accepted_by=AccountFactory()
        )
    ...
    def test_invite_duplicate_constraint_edge_cases(self):
        ...
        accepted_invite = InviteFactory(
            organization=org, email=email, status="accepted"
        )
...
class TestSerializerEdgeCases:
    ...
    def test_accept_invite_with_nonexistent_token(self):
        ...
        serializer = AcceptInviteSerializer(data={"token": "nonexistent_token_12345"})
