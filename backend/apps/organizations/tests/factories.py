...
class OrganizationFactory(DjangoModelFactory):
    ...
    name = factory.LazyFunction(lambda: Faker().company()[:100])  # Truncate to max 100 chars
    ...
    creator_name = factory.LazyFunction(lambda: Faker().name()[:255])  # Truncate to max 255 chars
    ...
    sub_domain = factory.LazyAttribute(
        lambda obj: _generate_subdomain(obj.name)
    )
...
class InviteFactory(DjangoModelFactory):
    ...
    expires_at = factory.LazyFunction(
        lambda: timezone.now() + timezone.timedelta(
...
class ExpiredInviteFactory(InviteFactory):
    ...
    status = 'expired'
    expires_at = factory.LazyFunction(lambda: timezone.now() - timezone.timedelta(days=1))


class AcceptedInviteFactory(InviteFactory):
    """Factory for accepted invites."""
    status = 'accepted'
    accepted_at = factory.LazyFunction(lambda: timezone.now() - timezone.timedelta(days=1))
