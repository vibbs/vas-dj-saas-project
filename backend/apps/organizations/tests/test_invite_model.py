...
from apps.organizations.tests.factories import InviteFactory, OrganizationFactory
...
class TestInviteModel:
    ...
    def test_string_representation(self):
        ...
        invite = InviteFactory(email="test@example.com", status="pending")
    ...
    def test_resend_pending_invite(self):
        ...
        with freeze_time("2024-01-01 12:00:00"):
            ...
            invite = InviteFactory(
                status="pending", token=old_token, expires_at=old_expires
            )
    ...
    def test_duplicate_pending_invites_not_allowed(self):
        ...
        InviteFactory(organization=org, email=email, status="pending")

        with pytest.raises(ValidationError, match="already exists"):
            invite = InviteFactory.build(
                organization=org, email=email, status="pending"
            )
            invite.full_clean()
    ...
    def test_different_status_invites_allowed(self):
        ...
        InviteFactory(organization=org, email=email, status="accepted")

        # Should be able to create new pending invite
        new_invite = InviteFactory(organization=org, email=email, status="pending")
    ...
    def test_user_already_member_validation(self):
        ...
        invite = InviteFactory.build(
            organization=org, email=user.email, status="pending"
        )
...
class TestInviteManager:
    ...
    def test_valid_filter(self):
        ...
        valid_invite = InviteFactory(
            organization=org, status="pending", expires_at=future_time
        )
        expired_invite = InviteFactory(
            organization=org, status="pending", expires_at=past_time
        )
        accepted_invite = InviteFactory(
            organization=org, status="accepted", expires_at=future_time
        )
