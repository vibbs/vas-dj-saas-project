...
class TestCompleteInviteWorkflow:
    ...
    def test_full_invite_workflow_success(self):
        ...
        create_serializer = CreateInviteSerializer(
            data=invite_data, context={"organization": org, "invited_by": owner}
        )
    ...
    def test_invite_workflow_with_existing_user(self):
        ...
        invite = InviteFactory(
            organization=org, invited_by=owner, email=existing_user.email, role="member"
        )
    ...
    def test_invite_workflow_with_plan_limits(self):
        ...
        invite = InviteFactory(
            organization=org, invited_by=owner, email=new_user.email, role="member"
        )
    ...
    def test_invite_expiry_workflow(self):
        ...
        invite = InviteFactory(
            organization=org, invited_by=owner, expires_at=past_time, status="pending"
        )
    ...
    def test_invite_resend_workflow(self):
        ...
        invite = InviteFactory(organization=org, invited_by=owner, status="pending")
...
class TestMembershipManagementWorkflow:
    ...
    def test_ownership_transfer_workflow(self):
        ...
        admin_membership = OrganizationMembershipFactory(
            organization=org, user=future_owner, role="admin", status="active"
        )
    ...
    def test_bulk_member_management_workflow(self):
        ...
        for i in range(10):
            ...
            membership = OrganizationMembershipFactory(
                organization=org, user=user, role="member", status="active"
            )
        ...
        assert (
            org.memberships.filter(status="active").count() == 11
        )  # 10 members + owner
...
class TestConcurrentMembershipWorkflow:
    ...
    def test_concurrent_invite_acceptance(self):
        ...
        invite = InviteFactory(
            organization=org, invited_by=owner, email="shared@example.com"
        )
...
class TestComplexOrganizationScenarios:
    ...
    def test_organization_merger_scenario(self):
        ...
        new_membership = OrganizationMembershipFactory(
            organization=org1, user=owner2, role="admin"
        )
        ...
        for membership in org2_members:
            ...
            OrganizationMembershipFactory(
                organization=org1, user=membership.user, role=membership.role
            )
    ...
    def test_seasonal_team_management(self):
        ...
        for i in range(5):
            ...
            invite = InviteFactory(
                ...
                email=user.email,
                role="member",
                message="Welcome to our seasonal team!",
            )
    ...
    def test_organization_lifecycle_complete(self):
        ...
        org = OrganizationFactory(name="Startup Inc", plan="free_trial", on_trial=True)
        ...
        cofounder_invite = InviteFactory(
            organization=org, invited_by=founder, email=cofounder.email, role="owner"
        )
        ...
        for i in range(8):  # Will hit free trial limit
            ...
            if i < 3:  # First 3 succeed
                invite = InviteFactory(
                    organization=org, invited_by=founder, email=user.email
                )
                membership = invite.accept(user)
