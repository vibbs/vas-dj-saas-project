...
class TestOrganizationMembershipSerializer:
    ...
    def test_update_membership(self):
        ...
        serializer = OrganizationMembershipSerializer(
            membership, data=data, partial=True
        )
        assert serializer.is_valid()
...
class TestInviteSerializer:
    ...
    def test_serialize_invite(self):
        ...
        inviter = AccountFactory(
            email="inviter@example.com", first_name="Jane", last_name="Smith"
        )
    ...
    def test_serialize_invite_computed_fields(self):
        ...
        valid_invite = InviteFactory(
            status="pending", expires_at=future_time, token="valid_token"
        )
        ...
        # Expired invite
        expired_invite = InviteFactory(status="pending", expires_at=past_time)
...
class TestCreateInviteSerializer:
    ...
    def test_create_valid_invite(self):
        ...
        serializer = CreateInviteSerializer(
            data=data, context={"organization": org, "invited_by": inviter}
        )
...
class TestDeprecatedOrganizationCreateSerializer:
    ...
    def test_create_organization_raises_error(self):
        ...
        with pytest.raises(
            DRFValidationError, match="Direct organization creation is deprecated"
        ):
            serializer.save()
...
class TestSerializerIntegration:
    ...
    def test_full_invite_workflow(self):
        ...
        # 1. Create invite
        create_data = {"email": invitee.email, "role": "admin", "message": "Join us!"}

        create_serializer = CreateInviteSerializer(
            data=create_data, context={"organization": org, "invited_by": inviter}
        )
    ...
    def test_membership_management_workflow(self):
        ...
        update_serializer = MembershipUpdateSerializer(
            admin, data=update_data, partial=True
        )
        ...
        suspend_serializer = MembershipUpdateSerializer(
            updated_membership, data=suspend_data, partial=True
        )
