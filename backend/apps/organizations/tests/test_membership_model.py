...
class TestOrganizationMembershipModel:
    ...
    def test_suspend_last_owner_fails(self):
        ...
        with pytest.raises(ValidationError, match="Cannot suspend the last active owner"):
            owner.suspend()
    ...
    def test_change_role_owner_to_admin_fails_if_last_owner(self):
        ...
        with pytest.raises(ValidationError, match="must have at least one active owner"):
    ...
    def test_get_permissions_admin(self):
        ...
        expected_permissions = [
            'view_organization', 'view_members', 'manage_members', 'invite_members'
        ]
        assert set(permissions) == set(expected_permissions)
    ...
    def test_get_permissions_owner(self):
        ...
        expected_permissions = [
            'view_organization', 'view_members', 'manage_members', 'invite_members',
            'manage_billing', 'delete_organization', 'manage_settings'
        ]
        assert set(permissions) == set(expected_permissions)
    ...
    def test_clean_active_without_user_fails(self):
        ...
        with pytest.raises(ValidationError, match="Active memberships must have a user"):
            membership.clean()
...
class TestMembershipBusinessLogic:
    ...
    def test_bulk_member_management(self):
        ...
        assert org.memberships.filter(status='active').count() == 6  # all members + owner
