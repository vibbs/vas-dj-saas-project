...
class TestAuthenticationSecurity:
    ...
    def test_unverified_email_cannot_login(self):
        ...
        assert 'email' in str(response.data).lower() or 'verif' in str(response.data).lower()
...
class TestEmailVerificationSecurity:
    ...
    def test_email_verification_token_is_single_use(self):
        ...
        response1 = self.client.post('/api/v1/auth/verify-email/', {
            'token': token
        })
        assert response1.status_code == status.HTTP_200_OK

        # Try to use same token again - should fail
        response2 = self.client.post('/api/v1/auth/verify-email/', {
            'token': token
        })
...
class TestSQLInjectionProtection:
    ...
    def test_sql_injection_in_search_parameters(self):
        ...
        for malicious_input in malicious_inputs:
            response = self.client.get(f'/api/v1/accounts/users/?search={malicious_input}')
