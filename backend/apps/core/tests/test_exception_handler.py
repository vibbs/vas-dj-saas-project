...
from apps.core.exceptions.handler import (
    _convert_drf_exception_to_rfc7807,
...
class TestRFC7807ExceptionHandler:
    ...
    def test_drf_validation_error_handling(
        self, sample_validation_error, mock_view_context
    ):
    ...
    def test_nested_validation_error_flattening(
        self, sample_nested_validation_error, mock_view_context
    ):
        """Test flattening of deeply nested validation errors."""
        response = rfc7807_exception_handler(
            sample_nested_validation_error, mock_view_context
        )
    ...
    def test_drf_exception_conversion(self, mock_view_context):
        ...
        with patch(
            "apps.core.exceptions.handler.drf_exception_handler"
        ) as mock_drf_handler:
            mock_response = Mock()
    ...
    def test_request_context_in_response(self):
        ...
        exc = BaseHttpException(
            type="https://docs.yourapp.com/problems/test", title="Test", status=400
        )

        context = {"request": Mock(path="/api/test/endpoint/")}
...
class TestDRFExceptionConversion:
    ...
    def test_not_authenticated_conversion(self):
        ...
        assert (
            result.type == "https://docs.yourapp.com/problems/authentication-required"
        )
...
class TestValidationErrorFlattening:
    ...
    def test_flatten_with_prefix_path(self):
        ...
        detail = {"email": ["Invalid email"], "nested": {"field": ["Error"]}}
    ...
    def test_flatten_complex_real_world_errors(self):
        ...
        pref_issues = [
            i for i in issues if len(i["path"]) >= 4 and "preferences" in str(i["path"])
        ]
