...
from apps.core.code_registry import (
    CodeRegistry, CodeRegistryError, REGISTRY
)
from apps.core.codes import BaseAPICodeMixin, BaseAPICodeEnum
...
class TestCodeRegistry:
    ...
    def test_load_app_codes_no_module(self):
        ...
        with patch('importlib.import_module') as mock_import:
            mock_import.side_effect = ModuleNotFoundError("No module named 'test_app.codes'")
    ...
    def test_register_code_duplicate(self):
        ...
        with pytest.raises(CodeRegistryError) as exc_info:
            registry._register_code('VDJ-TEST-SUCCESS-200', 'other_app', 'OtherCodes.SUCCESS')
    ...
    def test_load_app_error_catalog_no_file(self):
        ...
        with patch('importlib.import_module') as mock_import:
            ...
            with patch('pathlib.Path') as mock_path:
                ...
                mock_path_instance.parent.__truediv__ = Mock(return_value=mock_catalog_path)
            ...
            with patch('apps.core.code_registry.import_module') as mock_import:
                def import_side_effect(module_name):
                    if module_name.endswith('.codes'):
                        ...
                        if 'test_app1' in module_name:
                            ...
                            class App2TestCodes(BaseAPICodeMixin, metaclass=App2EnumMeta):
                                __name__ = 'TestCodes'
                                SUCCESS_200 = app2_member1
                                ERROR_400 = app2_member2
                            
                            App2TestCodes.__members__ = {'SUCCESS_200': app2_member1, 'ERROR_400': app2_member2}
                        ...
                        # Mock app module
                        mock_app = Mock()
                        mock_app.__file__ = f'/app/{module_name.split(".")[0]}/__init__.py'
                        return mock_app
                ...
                with patch('apps.core.code_registry.pathlib.Path') as mock_path:
                    ...
                    with patch('apps.core.code_registry.yaml') as mock_yaml:
                        ...
                        assert registry._loaded is True
                        assert len(registry.codes) == 6  # 3 codes per app (2 enum + 1 simple)
                        assert len(registry.problem_types) == 2  # Only first app's problem types due to duplicates
...
class TestCodeRegistryErrorHandling:
    ...
    def test_code_validation_edge_cases(self):
        ...
        with patch('apps.core.codes.BaseAPICodeMixin.validate_code') as mock_validate:
            ...
            with pytest.raises(CodeRegistryError) as exc_info:
                registry._register_code('INVALID-FORMAT', 'test_app', 'TestCodes.INVALID')
