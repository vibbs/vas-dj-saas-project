...
class TestUserRegistrationFlow:
    ...
    def test_registration_duplicate_email_fails(self):
        ...
        email_issues = [issue for issue in response.data['issues'] if 'email' in issue.get('path', [])]
    ...
    def test_registration_password_mismatch_fails(self):
        ...
        password_issues = [issue for issue in response.data['issues'] 
                         if any(field in issue.get('path', []) for field in ['password', 'password_confirm', 'non_field_errors'])]
    ...
    def test_registration_weak_password_fails(self):
        ...
        password_issues = [issue for issue in response.data['issues'] if 'password' in issue.get('path', [])]
        assert len(password_issues) > 0
...
class TestRegistrationWithCustomOrganization:
    ...
    def test_registration_with_invalid_subdomain_fails(self):
        ...
        for invalid_subdomain in invalid_subdomains:
            ...
            subdomain_issues = [issue for issue in response.data['issues'] if 'preferred_subdomain' in issue.get('path', [])]
    ...
    def test_registration_handles_long_organization_names(self):
        ...
        if response.status_code == status.HTTP_201_CREATED:
            ...
            org_name_issues = [issue for issue in response.data['issues'] if 'organization_name' in issue.get('path', [])]
