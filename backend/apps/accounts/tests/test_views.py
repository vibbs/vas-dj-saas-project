...
class TestAccountViews:
    ...
    def test_get_current_user_profile_authenticated(
    ...
    def test_get_current_user_profile_unauthenticated(self, api_client):
        ...
        assert (
            response.data.get("code") == "VDJ-AUTH-LOGIN-401"
    ...
    def test_get_user_auth_providers(self, authenticated_api_client, user):
        ...
        url = reverse(
            "account-auth-providers", kwargs={"pk": user.id}
        )  # Based on AccountViewSet router
        response = authenticated_api_client.get(url)
...
class TestAuthenticationViews:
    ...
    def test_login_with_valid_credentials(self, api_client):
        ...
        data = {"email": user.email, "password": "testpassword123"}  # From factory
    ...
    def test_login_with_unverified_account(self, api_client):
        ...
        assert (
            "email_verification" in str(response.data).lower()
            or "verify" in str(response.data).lower()
        )
