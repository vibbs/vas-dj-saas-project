...
class TestUserLoginFlow:
    ...
    def test_login_with_unverified_email_fails(self):
        ...
        assert error_found, f"Email verification error not found in response: {response.data}"
    ...
    def test_login_with_invalid_credentials_fails(self):
        ...
        assert response.data.get('code') == 'VDJ-ACC-CREDS-401'  # Invalid credentials error
    ...
    def test_login_missing_credentials_fails(self):
        ...
        # Missing email
        response = self.api_client.post(self.login_url, {'password': 'test123'}, format='json')
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.data.get('code') == 'VDJ-ACC-MISSING-400'
        
        # Missing password
        response = self.api_client.post(self.login_url, {'email': 'test@example.com'}, format='json')
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert response.data.get('code') == 'VDJ-ACC-MISSING-400'
...
class TestJWTTokenOperations:
    ...
    def setup_method(self):
        ...
        # Login to get tokens
        login_response = self.api_client.post(reverse('login'), {
            'email': 'tokenuser@example.com',
            'password': 'testpassword123'
        }, format='json')
    ...
    def test_token_refresh_success(self):
        ...
        assert response_data['refresh'] == self.refresh_token  # Depends on rotation policy
    ...
    def test_token_refresh_invalid_token_fails(self):
        ...
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        assert response.data.get('code') == 'VDJ-TOKEN-REFRESH-401'  # Invalid refresh token error
    ...
    def test_after_logout_refresh_token_is_blacklisted(self):
        ...
        # Logout first
        self.api_client.credentials(HTTP_AUTHORIZATION=f'Bearer {self.access_token}')
        logout_response = self.api_client.post(logout_url, {
            'refresh': self.refresh_token
        }, format='json')
        assert logout_response.status_code == status.HTTP_200_OK
        
        # Try to refresh with blacklisted token
        refresh_response = self.api_client.post(refresh_url, {
            'refresh': self.refresh_token
        }, format='json')
...
class TestEmailVerificationFlow:
    ...
    def test_email_verification_success(self):
        ...
        response = self.api_client.post(verify_url, {
            'token': token
        }, format='json')
    ...
    def test_email_verification_invalid_token_fails(self):
        ...
        token_issues = [issue for issue in response.data['issues'] if 'token' in issue.get('path', [])]
    ...
    def test_user_can_login_after_email_verification(self):
        ...
        # Verify email
        token = user.generate_email_verification_token()
        verify_response = self.api_client.post(reverse('auth-verify-email'), {
            'token': token
        }, format='json')
