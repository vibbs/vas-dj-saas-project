...
class TestSecurityEdgeCases:
    ...
    def test_registration_sql_injection_attempts(self):
        ...
        for injection_string in injection_attempts:
            ...
            if response.status_code == status.HTTP_201_CREATED:
                ...
                assert user.first_name == injection_string  # Should store as-is (Django ORM protects)
            else:
    ...
    def test_login_timing_attack_resistance(self):
        ...
        response1 = self.api_client.post(login_url, {
        ...
        start_time = time.time()
        response2 = self.api_client.post(login_url, {
    ...
    def test_password_requirements_enforced(self):
        ...
        for weak_password in weak_passwords:
            ...
            password_issues = [issue for issue in response.data['issues'] 
                             if 'password' in issue.get('path', [])]
            assert len(password_issues) > 0, f"No password validation issues found for {weak_password}: {response.data}"
    ...
    def test_email_verification_token_security(self):
        ...
        response = self.api_client.post(verify_url, {'token': original_token}, format='json')
        assert response.status_code == status.HTTP_400_BAD_REQUEST
...
class TestDataConsistencyEdgeCases:
    ...
    def test_registration_partial_failure_rollback(self):
        ...
        with patch('apps.accounts.models.Account.send_verification_email', side_effect=Exception('Email service down')):
            ...
            assert OrganizationMembership.objects.count() == initial_membership_count + 1
    ...
    def test_organization_subdomain_uniqueness_race_condition(self):
        ...
        emails = [f'user1-{test_id}@example.com', f'user2-{test_id}@example.com', f'user3-{test_id}@example.com']
        ...
        successful_responses = [r for r in responses if r.status_code == status.HTTP_201_CREATED]
        failed_responses = [r for r in responses if r.status_code == status.HTTP_400_BAD_REQUEST]
