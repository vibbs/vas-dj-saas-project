...
class AccountAdmin(UserAdmin):
    ...
    list_display = (
        ...
        'is_email_verified', 'is_org_admin', 'date_joined'
    )
    list_filter = (
        'status', 'is_active', 'is_email_verified', 'is_org_admin', 
        'is_org_creator', 'role', 'gender', 'date_joined'
    )
    search_fields = ('email', 'first_name', 'last_name')
    readonly_fields = (
        'date_joined', 'last_login', 'email_verification_token_expires',
        'abbreviated_name', 'full_name', 'is_admin'
    )
    ...
    fieldsets = (
        ...
        ('Authentication', {
            'fields': ('password', 'is_active', 'is_email_verified', 'is_phone_verified', 'is_2fa_enabled')
        }),
        ('Email Verification', {
        ...
        ('Roles & Permissions', {
            'fields': (
                'role', 'status', 'is_org_admin', 'is_org_creator',
        ...
        ('Metadata', {
            'fields': ('date_joined', 'last_login', 'abbreviated_name', 'full_name', 'is_admin'),
            'classes': ('collapse',)
        })
    )
...
class AccountAuthProviderAdmin(admin.ModelAdmin):
    ...
    search_fields = ('user__email', 'user__first_name', 'user__last_name', 'email', 'provider_user_id')
    readonly_fields = ('linked_at',)
    ordering = ('-linked_at',)
