...
class ComplexFeatureFlagScenarioFactory:
    ...
    def create_multi_tier_access(organization=None):
        ...
        # Create access rules
        user_rule = FeatureAccessFactory.create_for_user(
            regular_user, feature=flag, enabled=True
        )
        role_rule = FeatureAccessFactory.create_for_role(
            "ADMIN", feature=flag, enabled=True
        )

        org_rule = None
        if organization:
            org_rule = FeatureAccessFactory.create_for_organization(
                organization, feature=flag, enabled=True
            )
    ...
    def create_progressive_onboarding_scenario():
        ...
        verified_progress = UserOnboardingProgressFactory.create_email_verified(
            user=verified_user
        )
        profile_progress = UserOnboardingProgressFactory.create_profile_setup(
            user=profile_user
        )
        org_progress = UserOnboardingProgressFactory.create_organization_created(
            user=org_user
        )
        advanced_progress = UserOnboardingProgressFactory.create_advanced(
            user=advanced_user
        )
        completed_progress = UserOnboardingProgressFactory.create_completed(
            user=completed_user
        )
    ...
    def create_rollout_scenario(users_count=10):
        ...
        """
        flag = FeatureFlagFactory.create_with_rollout(
            percentage=50, key="rollout_feature"
        )
        users = [
            UserFactory(email=f"rollout_user_{i}@example.com")
            for i in range(users_count)
        ]

        return {"flag": flag, "users": users}
    ...
    def create_time_based_scenario():
        ...
        scheduled_flag = FeatureFlagFactory(
            key="future_feature", is_enabled_globally=True, active_from=future_time
        )

        # Expired flag (past expiration)
        expired_flag = FeatureFlagFactory(
            key="expired_feature", is_enabled_globally=True, active_until=past_time
        )
...
class BatchFactory:
    ...
    def create_feature_flags(count=50):
        ...
        return [FeatureFlagFactory(key=f"perf_flag_{i}") for i in range(count)]

    @staticmethod
    def create_users(count=100):
        """Create a batch of users."""
        return [UserFactory(email=f"perf_user_{i}@example.com") for i in range(count)]
