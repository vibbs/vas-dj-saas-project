...
from ..utils.helpers import (
    ...
    create_feature_flag,
    disable_flag_for_user,
    enable_flag_for_user,
    get_enabled_features_for_user,
    get_feature_flag_statistics,
    get_user_feature_flags,
    get_user_onboarding_info,
    is_feature_enabled_for_user,
    trigger_onboarding_action,
)
from ..utils.mixins import (
    FeatureContextMixin,
    FeatureGatedMixin,
    FeatureRequiredMixin,
    MultipleFeatureGatedMixin,
)
...
class TestOnboardingStageRequiredDecorator:
    ...
    def test_decorator_allows_advanced_stage(self, user):
        ...
        UserOnboardingProgressFactory(
            user=user, current_stage=OnboardingStageTypes.PROFILE_SETUP.value
        )

        @onboarding_stage_required(
            OnboardingStageTypes.EMAIL_VERIFIED.value, allow_advanced=True
        )
        def test_view(request):
            return "success"
...
class TestFeatureContextMixin:
    ...
    def test_mixin_get_context_data(self, user):
        ...
        with patch(
            "django.views.generic.base.TemplateView.get_context_data"
        ) as mock_context:
...
class TestHelperFunctions:
    ...
    def test_trigger_onboarding_action(self, user):
        ...
        result = trigger_onboarding_action(
            user, "email_verified", {"method": "email_link"}
        )
    ...
    def test_enable_flag_for_user_helper(self, user):
        ...
        rule = FeatureAccess.objects.filter(
    ...
    def test_clear_feature_flag_cache_user(self, user):
        ...
        with patch(
            "apps.feature_flags.services.FeatureFlagService.invalidate_user_cache"
        ) as mock_clear:
    ...
    def test_clear_feature_flag_cache_flag(self):
        ...
        with patch(
            "apps.feature_flags.services.FeatureFlagService.invalidate_flag_cache"
        ) as mock_clear:
...
class TestErrorHandling:
    ...
    def test_decorator_error_handling_development(self, user):
        ...
        with patch(
            "apps.feature_flags.services.feature_service.FeatureFlagService.is_feature_enabled"
        ) as mock_service:
    ...
    def test_decorator_error_handling_production(self, user):
        ...
        with patch(
            ...
            with patch("django.conf.settings.DEBUG", False):
                with pytest.raises(
                    PermissionDenied, match="Feature access could not be verified"
                ):
                    test_view(request)
