...
class TestFeatureFlagViewSet:
    ...
    def test_list_feature_flags_authenticated(self, authenticated_api_client):
    ...
    def test_create_feature_flag_non_admin(self, authenticated_api_client, valid_flag_data):
    ...
    def test_update_feature_flag_non_admin(self, authenticated_api_client, feature_flag):
    ...
    def test_feature_flag_filtering(self, authenticated_api_client):
        """Test filtering feature flags."""
        FeatureFlagFactory(is_enabled_globally=True, key='enabled_flag')
        FeatureFlagFactory(is_enabled_globally=False, key='disabled_flag')
    ...
    def test_feature_flag_search(self, authenticated_api_client):
        """Test searching feature flags."""
        FeatureFlagFactory(name='Analytics Feature', key='analytics')
        FeatureFlagFactory(name='Reporting Feature', key='reporting')
    ...
    def test_feature_flag_ordering(self, authenticated_api_client):
        """Test ordering feature flags."""
        flag1 = FeatureFlagFactory(name='B Feature')
        flag2 = FeatureFlagFactory(name='A Feature')
...
class TestUserFeatureFlagsView:
    ...
    def test_get_user_flags_authenticated(self, authenticated_api_client, user):
    ...
    def test_get_user_flags_with_organization(self, authenticated_api_client, user, organization):
        ...
        url = reverse('feature_flags:user-feature-flags')
        response = authenticated_api_client.get(url, {'organization_id': str(organization.id)})
        
        assert response.status_code == status.HTTP_200_OK
        assert flag.key in response.data['enabled_flags']
    
    def test_get_user_flags_specific_keys(self, authenticated_api_client, user):
        """Test getting specific feature flags for user."""
        flag1 = FeatureFlagFactory(is_enabled_globally=True)
        flag2 = FeatureFlagFactory(is_enabled_globally=False)
        
        url = reverse('feature_flags:user-feature-flags')
        response = authenticated_api_client.get(url, {'flags': f'{flag1.key},{flag2.key}'})
...
class TestFeatureFlagToggleView:
    ...
    def test_toggle_flag_for_user_admin(self, admin_api_client, feature_flag, user):
        ...
        toggle_data = {
            'enabled': True,
            'reason': 'Testing user toggle'
        }
        
        url = reverse('feature_flags:feature-flag-user-toggle', 
    ...
    def test_toggle_flag_for_user_non_admin(self, authenticated_api_client, feature_flag, user):
...
class TestFeatureAccessViewSet:
    ...
    def test_update_access_rule_admin(self, admin_api_client, user_access_rule):
        ...
        url = reverse('feature_flags:featureaccess-detail', kwargs={'pk': user_access_rule.id})
        response = admin_api_client.patch(url, update_data, format='json')
    ...
    def test_filter_access_rules_by_feature(self, admin_api_client, multiple_feature_flags):
        ...
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data['data']) == 1
        assert response.data['data'][0]['id'] == rule1.id
    
    def test_filter_access_rules_by_user(self, admin_api_client, feature_flag, multiple_users):
...
class TestBulkAccessRuleView:
    ...
    def test_bulk_create_access_rules_admin(self, admin_api_client, feature_flag, multiple_users):
        """Test bulk creating access rules as admin."""
        bulk_data = {
            'feature_id': feature_flag.id,
            'rules': [
                {'user_id': multiple_users[0].id, 'enabled': True, 'reason': 'Bulk rule 1'},
                {'user_id': multiple_users[1].id, 'enabled': False, 'reason': 'Bulk rule 2'},
            ]
        }
    ...
    def test_bulk_create_access_rules_non_admin(self, authenticated_api_client, feature_flag):
    ...
    def test_bulk_create_invalid_data(self, admin_api_client):
        ...
        bulk_data = {
            'rules': [{'enabled': True}]  # Missing feature_id
        }
    ...
    def test_bulk_update_access_rules_admin(self, admin_api_client, feature_flag, multiple_users):
        """Test bulk updating access rules as admin."""
        rule1 = FeatureAccessFactory(feature=feature_flag, user=multiple_users[0], enabled=True)
        rule2 = FeatureAccessFactory(feature=feature_flag, user=multiple_users[1], enabled=True)
    ...
    def test_bulk_delete_access_rules_admin(self, admin_api_client, feature_flag, multiple_users):
...
class TestUserOnboardingProgressViewSet:
    ...
    def test_retrieve_own_onboarding_progress(self, authenticated_api_client, user):
        """Test retrieving own onboarding progress."""
        progress = UserOnboardingProgressFactory(user=user)
        
        url = reverse('feature_flags:onboardingprogress-detail', kwargs={'pk': progress.id})
        response = authenticated_api_client.get(url)
        
        assert response.status_code == status.HTTP_200_OK
        assert response.data['user'] == user.id
    
    def test_retrieve_other_onboarding_progress_non_admin(self, authenticated_api_client):
    ...
    def test_update_onboarding_progress_admin(self, admin_api_client, user):
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data['current_stage'] == OnboardingStageTypes.EMAIL_VERIFIED.value
    
    def test_update_own_onboarding_progress(self, authenticated_api_client, user):
        """Test updating own onboarding progress."""
        progress = UserOnboardingProgressFactory(user=user)
        
        update_data = {
            'custom_data': {'self_updated': True}
        }
    ...
    def test_filter_onboarding_by_stage(self, admin_api_client, multiple_users):
        ...
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data['data']) == 1
        assert response.data['data'][0]['current_stage'] == OnboardingStageTypes.EMAIL_VERIFIED.value
...
class TestOnboardingActionView:
    ...
    def test_process_onboarding_action_authenticated(self, authenticated_api_client, user):
    ...
    def test_process_onboarding_action_with_service_error(self, authenticated_api_client):
        """Test handling service errors in onboarding action."""
        with patch('apps.feature_flags.services.OnboardingService.handle_user_action',
                  side_effect=Exception('Service error')):
            action_data = {'action': 'email_verified'}
...
class TestAPIPagination:
    ...
    def test_feature_flags_pagination(self, authenticated_api_client):
        """Test pagination for feature flags list."""
        # Create more flags than the page size
        for i in range(15):
            FeatureFlagFactory(key=f'flag_{i}')
...
class TestAPIValidation:
    ...
    def test_feature_flag_creation_validation(self, admin_api_client):
        ...
        invalid_cases = [
            ...
            {'key': 'test', 'name': 'Test', 'rollout_percentage': -1},  # Negative percentage
        ]
...
class TestAPIPermissions:
    ...
    def test_admin_only_endpoints(self, authenticated_api_client, admin_api_client):
        ...
        admin_only_urls = [
            ...
            reverse('feature_flags:onboardingprogress-list'),
...
class TestAPIErrorHandling:
    ...
    def test_database_error_handling(self, authenticated_api_client):
        ...
        with patch('apps.feature_flags.models.FeatureFlag.objects.all',
