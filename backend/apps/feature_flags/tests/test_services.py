...
from ..models import FeatureAccess, UserOnboardingProgress
...
class TestFeatureFlagService:
    ...
    def test_is_feature_enabled_globally_disabled_flag(
        self, user, feature_flag_service
    ):
    ...
    def test_is_feature_enabled_user_specific_disabled(
        self, user, feature_flag_service
    ):
    ...
    def test_is_feature_enabled_organization_access(
        self, organization, feature_flag_service
    ):
    ...
    def test_is_feature_enabled_scheduled_future(self, user, feature_flag_service):
        ...
        flag = FeatureFlagFactory(is_enabled_globally=True, active_from=future_time)
    ...
    def test_is_feature_enabled_expired(self, user, feature_flag_service):
        ...
        flag = FeatureFlagFactory(is_enabled_globally=True, active_until=past_time)
    ...
    def test_is_feature_enabled_cache_hit(self, user, cached_feature_flag_service):
        ...
        with patch.object(
            FeatureFlagCacheService, "get_user_flags", return_value={"test_flag": True}
        ):
    ...
    def test_is_feature_enabled_force_refresh(self, user, cached_feature_flag_service):
        ...
        with patch.object(
            FeatureFlagCacheService, "get_user_flags", return_value={"test_flag": False}
        ):
    ...
    def test_get_user_flags_specific_keys(self, user, feature_flag_service):
        ...
        flags = feature_flag_service.get_user_flags(
            user, flag_keys=[flag1.key, flag2.key]
        )
    ...
    def test_get_user_flags_cache_miss(
        self, user, cached_feature_flag_service, mock_redis_cache
    ):
    ...
    def test_create_feature_flag_duplicate_key(self, feature_flag_service):
        ...
        flag = feature_flag_service.create_feature_flag(
            key="existing_feature", name="Duplicate Feature"
        )
    ...
    def test_create_access_rule_success(self, user, feature_flag_service):
        ...
        rule = feature_flag_service.create_access_rule(
            flag.key, user=user, enabled=True, reason="Test rule"
        )
    ...
    def test_create_access_rule_nonexistent_flag(self, user, feature_flag_service):
        ...
        rule = feature_flag_service.create_access_rule("nonexistent_flag", user=user)
    ...
    def test_enable_flag_for_user(self, user, feature_flag_service):
        ...
        result = feature_flag_service.enable_flag_for_user(
            flag.key, user, reason="Testing user enablement"
        )

        assert result is True
        assert FeatureAccess.objects.filter(
            feature=flag, user=user, enabled=True
        ).exists()
    ...
    def test_disable_flag_for_user(self, user, feature_flag_service):
        ...
        result = feature_flag_service.disable_flag_for_user(
            flag.key, user, reason="Testing user disablement"
        )

        assert result is True
        assert FeatureAccess.objects.filter(
            feature=flag, user=user, enabled=False
        ).exists()
    ...
    def test_enable_flag_for_role(self, feature_flag_service):
        ...
        result = feature_flag_service.enable_flag_for_role(
            flag.key, "ADMIN", reason="Testing role enablement"
        )

        assert result is True
        assert FeatureAccess.objects.filter(
            feature=flag, role="ADMIN", enabled=True
        ).exists()

    def test_invalidate_user_cache(self, user, cached_feature_flag_service):
        """Test invalidating user cache."""
        with patch.object(
            FeatureFlagCacheService, "invalidate_user_flags"
        ) as mock_invalidate:
    ...
    def test_invalidate_flag_cache(self, cached_feature_flag_service):
        ...
        with patch.object(
            FeatureFlagCacheService, "invalidate_all_flag_caches"
        ) as mock_invalidate:
    ...
    def test_is_feature_enabled_error_handling(self, user, feature_flag_service):
        ...
        with patch(
            "apps.feature_flags.models.FeatureFlag.objects.get",
    ...
    def test_update_user_onboarding_error_handling(self, user, feature_flag_service):
        ...
        with patch(
            "apps.feature_flags.models.UserOnboardingProgress.objects.get_or_create",
            side_effect=Exception("DB error"),
        ):
...
class TestOnboardingService:
    ...
    def test_advance_user_stage_cache_invalidation(
        self, user, cached_onboarding_service
    ):
        """Test cache invalidation during stage advancement."""
        UserOnboardingProgressFactory(user=user)

        with patch.object(
            FeatureFlagCacheService, "invalidate_all_user_caches"
        ) as mock_invalidate:
    ...
    def test_handle_user_action_email_verified(self, user, onboarding_service):
        ...
        result = onboarding_service.handle_user_action(
            user, "email_verified", {"verification_method": "email_link"}
        )
    ...
    def test_handle_user_action_skip_stages(self, user, onboarding_service):
        ...
        result = onboarding_service.handle_user_action(
            user, "organization_created", {"org_name": "Test Org"}
        )
    ...
    def test_get_stage_info(self, onboarding_service):
        ...
        info = onboarding_service.get_stage_info(
            OnboardingStageTypes.EMAIL_VERIFIED.value
        )
    ...
    def test_check_requirement_has_organization(self, organization, onboarding_service):
        ...
        with patch.object(user, "get_primary_organization", return_value=organization):
            result = onboarding_service._check_requirement(
                user, progress, "has_organization"
            )

            assert result is True

    def test_check_requirement_organization_members(
        self, organization, onboarding_service
    ):
        ...
        with patch.object(user, "get_primary_organization", return_value=mock_org):
            result = onboarding_service._check_requirement(
                user, progress, "organization_has_members"
            )
    ...
    def test_check_requirement_custom_data(self, user, onboarding_service):
        ...
        progress = UserOnboardingProgressFactory(
            user=user, custom_data={"project_created": True}
        )

        result = onboarding_service._check_requirement(
            user, progress, "has_created_project"
        )
    ...
    def test_error_handling_check_requirements(self, user, onboarding_service):
        ...
        with patch.object(
            onboarding_service, "get_or_create_progress", side_effect=Exception("Error")
        ):
            can_advance, missing = onboarding_service.check_stage_requirements(
                user, "test_stage"
            )
