...
class FeatureFlagAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'name', 'key', 'is_enabled_globally', 'rollout_percentage',
        'active_status', 'access_rules_count', 'organization',
        'created_at', 'updated_at'
    ]
    list_filter = [
        'is_enabled_globally', IsActiveListFilter, 'is_permanent',
        'requires_restart', 'created_at', 'organization'
    ]
    search_fields = ['name', 'key', 'description']
    readonly_fields = [
        'id', 'created_at', 'updated_at', 'created_by', 'updated_by',
        'active_status', 'access_rules_count'
    ]
    ...
    fieldsets = (
        ...
        ('Metadata', {
            'fields': ('id', 'active_status', 'access_rules_count', 'created_at', 'updated_at', 'created_by', 'updated_by'),
            'classes': ('collapse',)
        })
    )
    ...
    def get_queryset(self, request):
        queryset = super().get_queryset(request)
        return queryset.select_related('organization', 'created_by', 'updated_by').prefetch_related('access_rules')
...
class FeatureAccessAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'feature_name', 'feature_key', 'target_display', 'enabled',
        'conditions_summary', 'reason', 'organization', 'created_at'
    ]
    list_filter = [
        'enabled', 'feature', 'created_at', 'organization',
        ('user', admin.RelatedOnlyFieldListFilter),
    ]
    search_fields = [
        'feature__name', 'feature__key', 'user__email',
        'role', 'reason'
    ]
    readonly_fields = [
        'id', 'created_at', 'updated_at', 'created_by', 'updated_by'
...
class OnboardingStageListFilter(SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        if self.value() == 'early':
            ...
            return queryset.filter(current_stage=OnboardingStageTypes.ONBOARDING_COMPLETE.value)
...
class UserOnboardingProgressAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'user_email', 'user_full_name', 'current_stage', 'progress_bar',
        'total_actions_completed', 'stage_started_at', 'is_complete',
        'organization'
    ]
    list_filter = [
        'current_stage', OnboardingStageListFilter, 'created_at',
        'onboarding_completed_at', 'organization',
        ('user', admin.RelatedOnlyFieldListFilter)
    ]
    search_fields = ['user__email', 'user__first_name', 'user__last_name']
    readonly_fields = [
        'id', 'progress_bar', 'is_complete', 'next_stage',
        'available_features_list', 'created_at', 'updated_at',
    ...
    fieldsets = (
        ...
        ('Progress', {
            'fields': ('current_stage', 'progress_bar', 'total_actions_completed',
                      'is_complete', 'next_stage')
        }),
        ('Stage History', {
        ...
        ('Timestamps', {
            'fields': ('stage_started_at', 'last_activity_at', 'onboarding_completed_at')
        }),
        ('Features', {
            'fields': ('available_features_list',),
            'classes': ('collapse',)
        }),
        ('Custom Data', {
            'fields': ('custom_data',),
    ...
    def progress_bar(self, obj):
        ...
        color = 'red' if percentage < 25 else 'orange' if percentage < 50 else 'yellow' if percentage < 75 else 'green'
