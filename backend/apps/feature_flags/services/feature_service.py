...
class FeatureFlagService:
    ...
    def is_feature_enabled(
        self, user, flag_key: str, organization=None, force_refresh: bool = False
    ) -> bool:
        """
        ...
        try:
            ...
            if self.use_cache and not force_refresh:
                cached_flags = self.cache_service.get_user_flags(
                    str(user.id), str(organization.id) if organization else None
                )
            ...
            if self.use_cache and result is not None:
                ...
                user_flags = (
                    self.cache_service.get_user_flags(
                        str(user.id), str(organization.id) if organization else None
                    )
                    or {}
                )
            ...
            logger.error(
                f"Error evaluating flag {flag_key} for user {user.id}: {str(e)}"
            )
            return False
    ...
    def get_enabled_flags(
        self, user, organization=None, flag_keys: list[str] | None = None
    ) -> list[str]:
        """
        ...
        try:
            with transaction.atomic():
                progress, created = UserOnboardingProgress.objects.get_or_create(
                    user=user, defaults={"current_stage": new_stage}
                )
    ...
    def disable_flag_for_user(self, flag_key: str, user, reason: str = "") -> bool:
        ...
        rule = self.create_access_rule(
            flag_key, user=user, enabled=False, reason=reason
        )
        return rule is not None
    ...
    def _evaluate_flag_for_user(
        self, user, flag_key: str, organization=None
    ) -> bool | None:
        """
        ...
        try:
            ...
            user_access = FeatureAccess.objects.filter(feature=flag, user=user).first()
            ...
            if organization:
                org_access = FeatureAccess.objects.filter(
                    feature=flag, organization=organization
                ).first()
    ...
    def _evaluate_all_flags_for_user(
        self, user, organization=None, flag_keys: list[str] | None = None
    ) -> dict[str, bool]:
        """
