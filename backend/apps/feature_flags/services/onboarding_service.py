...
class OnboardingService:
    ...
    def get_or_create_progress(self, user) -> UserOnboardingProgress:
        ...
        try:
            ...
            logger.error(
                f"Error getting/creating progress for user {user.id}: {str(e)}"
            )
            raise

    def advance_user_stage(
        self, user, new_stage: str, custom_data: dict | None = None
    ) -> bool:
        """
        ...
        try:
            ...
            logger.error(
                f"Error advancing user {user.id} to stage {new_stage}: {str(e)}"
            )
            return False

    def mark_stage_completed(
        self, user, stage: str, custom_data: dict | None = None
    ) -> bool:
        """
        ...
        try:
            ...
            logger.error(
                f"Error marking stage {stage} completed for user {user.id}: {str(e)}"
            )
            return False

    def check_stage_requirements(
        self, user, target_stage: str
    ) -> tuple[bool, list[str]]:
        """
        ...
        try:
            ...
            logger.debug(
                f"Stage requirements check for {user.id} -> {target_stage}: "
            ...
            logger.error(
                f"Error checking stage requirements for user {user.id}: {str(e)}"
            )
            return False, ["error_checking_requirements"]

    def auto_progress_user(self, user) -> str | None:
        """
        ...
        try:
            ...
            can_advance, missing_requirements = self.check_stage_requirements(
                user, next_stage
            )
            ...
            if can_advance:
                ...
                logger.debug(
                    f"User {user.id} cannot auto-progress. Missing: {missing_requirements}"
                )
    ...
    def handle_user_action(
        self, user, action: str, metadata: dict | None = None
    ) -> bool:
        """
        ...
        try:
            ...
            logger.debug(
                f"Processed action {action} for user {user.id} (no stage change)"
            )
            return True
    ...
    def get_user_progress_summary(self, user) -> dict[str, Any]:
        ...
        try:
            ...
            if next_stage:
                _, missing_requirements = self.check_stage_requirements(
                    user, next_stage
                )
    ...
    def _check_requirement(
        self, user, progress: UserOnboardingProgress, requirement: str
    ) -> bool:
        """
