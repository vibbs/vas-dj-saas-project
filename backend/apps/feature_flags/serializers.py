...
class FeatureFlagSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = FeatureFlag
        fields = [
            'id', 'key', 'name', 'description', 'is_enabled_globally',
            'rollout_percentage', 'is_permanent', 'requires_restart',
            'environments', 'active_from', 'active_until',
            'is_active_now', 'total_access_rules', 'enabled_access_rules',
            'created_at', 'updated_at', 'created_by', 'organization'
        ]
        read_only_fields = [
...
class FeatureFlagSummarySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = FeatureFlag
        fields = [
            'id', 'key', 'name', 'is_enabled_globally',
            'rollout_percentage', 'is_active_now', 'updated_at'
        ]
...
class FeatureAccessSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = FeatureAccess
        fields = [
            'id', 'feature', 'feature_key', 'feature_name',
            'user', 'user_email', 'role', 'enabled',
            'conditions', 'reason', 'applies_to_current_user',
            'created_at', 'updated_at', 'organization'
        ]
        read_only_fields = [
            'id', 'created_at', 'updated_at', 'feature_key',
            'feature_name', 'user_email', 'applies_to_current_user'
        ]
...
class UserOnboardingProgressSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'current_stage', 'completed_stages', 'total_actions_completed',
            'progress_percentage', 'is_complete', 'next_stage',
            'available_features', 'stage_started_at', 'last_activity_at',
            'onboarding_completed_at', 'custom_data', 'days_since_start',
            'created_at', 'updated_at', 'organization'
        ]
        read_only_fields = [
            'id', 'user_email', 'user_full_name', 'is_complete',
            'next_stage', 'available_features', 'days_since_start',
            'total_actions_completed', 'progress_percentage',
            'last_activity_at', 'onboarding_completed_at',
...
class FeatureFlagBulkUpdateSerializer(serializers.Serializer):
    ...
    flag_keys = serializers.ListField(
        child=serializers.CharField(),
        help_text="List of feature flag keys to update"
    )
    ...
    reason = serializers.CharField(
        required=False,
        allow_blank=True,
        help_text="Reason for the bulk update"
    )
    
    def validate_flag_keys(self, value):
        """Validate that all flag keys exist."""
        existing_keys = set(FeatureFlag.objects.filter(key__in=value).values_list('key', flat=True))
