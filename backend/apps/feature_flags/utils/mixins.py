...
class FeatureGatedMixin(LoginRequiredMixin):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not self.required_feature_flag:
            raise ValueError(f"{self.__class__.__name__} must define required_feature_flag")
    ...
    def _check_feature_flag(self, request):
        ...
        try:
            ...
            return service.is_feature_enabled(user, self.required_feature_flag, organization)
            
        except Exception as e:
            logger.error(f"Error checking feature flag {self.required_feature_flag}: {str(e)}")
    ...
    def _handle_feature_denied(self, request):
        ...
        logger.warning(f"Access denied to {self.required_feature_flag} for user {request.user.id}")
        
        # Try custom response method first
        if self.feature_flag_custom_response_method:
            custom_method = getattr(self, self.feature_flag_custom_response_method, None)
        ...
        raise PermissionDenied(f"Feature '{self.required_feature_flag}' is not enabled for your account")
...
class MultipleFeatureGatedMixin(LoginRequiredMixin):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not self.required_feature_flags:
            raise ValueError(f"{self.__class__.__name__} must define required_feature_flags")
    ...
    def _check_feature_flags(self, request):
        ...
        try:
            ...
            logger.error(f"Error checking feature flags {self.required_feature_flags}: {str(e)}")
    ...
    def _handle_multiple_features_denied(self, request, missing_flags):
        ...
        logger.warning(f"Access denied to {self.required_feature_flags} for user {request.user.id}. Missing: {missing_flags}")
...
class OnboardingStageRequiredMixin(LoginRequiredMixin):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not self.required_onboarding_stage:
            raise ValueError(f"{self.__class__.__name__} must define required_onboarding_stage")
    ...
    def _check_onboarding_stage(self, request):
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Invalid onboarding stage: {self.required_onboarding_stage} or {progress.current_stage}")
                return None
            ...
            logger.error(f"Error checking onboarding stage {self.required_onboarding_stage}: {str(e)}")
    ...
    def _handle_onboarding_denied(self, request, reason):
        ...
        raise PermissionDenied(f"Onboarding stage '{self.required_onboarding_stage}' required")
