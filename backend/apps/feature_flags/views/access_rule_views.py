...
from ..serializers import FeatureAccessSerializer, FeatureFlagBulkUpdateSerializer
...
class FeatureAccessViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_staff or user.is_superuser:
            pass  # No permission filtering for admins
    ...
    def get_permissions(self):
        ...
        if self.action in ["create", "update", "partial_update", "destroy", "list"]:
            # Only admins can manage and list access rules
            permission_classes = [IsAdminUser]
        elif self.action in ["retrieve"]:
            # Authenticated users can view specific rules they have access to
            permission_classes = [IsAuthenticated]
        else:
            permission_classes = [IsAuthenticated]
    ...
    @extend_schema(
        ...
        responses={
            ...
            400: OpenApiExample(
                "Missing Parameter", value={"error": "user_id parameter is required"}
            ),
    ...
    def for_user(self, request):
        ...
        Permission: Only admins or the user themselves can view access rules.
        """
        ...
        try:
            ...
            if not (
                ...
                or str(request.user.id) == str(user_id)
            ):
                return Response(
                    {
    ...
    @extend_schema(
        ...
        responses={
            ...
            400: OpenApiExample(
                "Missing Parameter", value={"error": "flag_key parameter is required"}
            ),
            404: OpenApiExample(
    ...
    def for_flag(self, request):
        ...
        try:
            ...
            return Response(
                {"error": "Feature flag not found"}, status=status.HTTP_404_NOT_FOUND
            )
        except Exception as e:
...
class BulkAccessRuleView(APIView):
        ...
        responses={
            ...
            207: OpenApiExample(
                ...
                value={
                    ...
                    "errors": [
                        "Failed to process analytics -> invalid_user_id: User not found"
                    ],
                },
            ),
            400: OpenApiExample(
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            try:
                ...
                response_data = {
                    "message": "Bulk operation completed",
                    "total_operations": len(data["flag_keys"])
                    * len(data["target_ids"]),
    ...
    @extend_schema(
        ...
        description="Update multiple access rules by their IDs. Apply the same changes to "
        "multiple rules in a single operation for efficient bulk management.",
        request={
            "application/json": {
                "type": "object",
                "properties": {
                    "rule_ids": {
                        ...
                        "description": "List of access rule IDs to update",
                    },
                    "updates": {
                        "type": "object",
                        "properties": {
                ...
                "required": ["rule_ids", "updates"],
            }
        },
        responses={
            200: OpenApiExample(
                "Bulk Update Success",
                value={"message": "Bulk update completed", "updated_count": 5},
            ),
            400: OpenApiExample(
                "Invalid Request", value={"error": "rule_ids and updates are required"}
            ),
        },
        tags=["Feature Flags"],
    )
    def patch(self, request):
        """
        Bulk update existing access rules.
        """
        ...
        if not rule_ids or not updates:
            return Response(
                {"error": "rule_ids and updates are required"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        try:
            # Get all rules that the user can modify
            rules = FeatureAccess.objects.filter(id__in=rule_ids)

            if not request.user.is_staff and not request.user.is_superuser:
                return Response(
                    {"error": "Only administrators can bulk update access rules"},
                    status=status.HTTP_403_FORBIDDEN,
                )
            ...
            return Response(
                {"message": "Bulk update completed", "updated_count": updated_count},
                status=status.HTTP_200_OK,
            )
    ...
    @extend_schema(
        ...
        description="Delete multiple access rules by their IDs. Remove multiple rules in a "
        "single operation for efficient bulk management.",
        request={
            "application/json": {
                "type": "object",
                "properties": {
                    "rule_ids": {
                        "type": "array",
                        "items": {"type": "string", "format": "uuid"},
                        "description": "List of access rule IDs to delete",
                    }
                },
                "required": ["rule_ids"],
            }
        ...
        responses={
            200: OpenApiExample(
                "Bulk Delete Success",
                value={"message": "Bulk delete completed", "deleted_count": 5},
            ),
            400: OpenApiExample(
                "Invalid Request", value={"error": "rule_ids is required"}
            ),
        },
        tags=["Feature Flags"],
    )
    def delete(self, request):
        """
        Bulk delete access rules.
        """
        rule_ids = request.data.get("rule_ids", [])

        if not rule_ids:
            return Response(
                {"error": "rule_ids is required"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            if not request.user.is_staff and not request.user.is_superuser:
                return Response(
                    {"error": "Only administrators can bulk delete access rules"},
                    status=status.HTTP_403_FORBIDDEN,
                )

            # Store cache invalidation data before deletion
            service = FeatureFlagService()
            cache_invalidation_data = []

            for rule in rules:
                cache_invalidation_data.append(
                    {"user": rule.user, "flag_key": rule.feature.key}
                )

            # Delete rules
            deleted_count, _ = rules.delete()

            # Invalidate caches
            for data in cache_invalidation_data:
                if data["user"]:
                    service.invalidate_user_cache(data["user"])
                service.invalidate_flag_cache(data["flag_key"])

            return Response(
                {"message": "Bulk delete completed", "deleted_count": deleted_count},
                status=status.HTTP_200_OK,
            )
