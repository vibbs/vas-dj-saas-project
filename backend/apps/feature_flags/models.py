...
class FeatureFlag(BaseFields):
    ...
    description = models.TextField(
        blank=True, help_text="Detailed description of what this feature flag controls"
    )
    ...
    active_until = models.DateTimeField(
        null=True, blank=True, help_text="Feature is active until this date (optional)"
    )
...
class FeatureAccess(BaseFields):
    ...
    feature = models.ForeignKey(
        FeatureFlag, on_delete=models.CASCADE, related_name="access_rules"
    )
    ...
    def clean(self):
        ...
        if not any([self.user, self.role]):
            raise ValidationError(_("At least one of user or role must be specified."))
    ...
    def check_conditions(self, user):
        ...
        if "requires_email_verified" in self.conditions:
            if (
                self.conditions["requires_email_verified"]
...
class UserOnboardingProgress(BaseFields):
    ...
    completed_stages = models.JSONField(
        default=list, help_text="List of completed onboarding stages"
    )

    # Progress tracking
    total_actions_completed = models.PositiveIntegerField(
        default=0, help_text="Total number of onboarding actions completed"
    )
    progress_percentage = models.PositiveIntegerField(
        default=0, help_text="Percentage of onboarding completion (0-100)"
    )

    # Timestamps for progression
    stage_started_at = models.DateTimeField(
        default=timezone.now, help_text="When the current stage was started"
    )
    last_activity_at = models.DateTimeField(
        auto_now=True, help_text="Last time user progressed in onboarding"
    )
    onboarding_completed_at = models.DateTimeField(
        null=True, blank=True, help_text="When onboarding was fully completed"
    )
    ...
    def is_onboarding_complete(self):
        ...
        return (
            self.current_stage == OnboardingStageTypes.ONBOARDING_COMPLETE.value
            or self.onboarding_completed_at is not None
        )
