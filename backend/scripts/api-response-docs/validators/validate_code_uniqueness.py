    ...
    for app_name in settings.INSTALLED_APPS:
        ...
        for attr_name in dir(codes_module):
            ...
            if (isinstance(attr_value, type) and 
                issubclass(attr_value, BaseAPICodeMixin) and 
                attr_value is not BaseAPICodeMixin and
                hasattr(attr_value, '__members__')):  # Ensure it's an actual enum
            ...
            elif isinstance(attr_value, str) and BaseAPICodeMixin.validate_code(attr_value):
    ...
    for i, code1 in enumerate(codes_list):
        ...
        for code2 in codes_list[i+1:]:
            ...
            if len(parts1) >= 3 and len(parts2) >= 3:
                if (parts1[1] == parts2[1] and  # Same module
                    parts1[-1] != parts2[-1] and  # Different HTTP code
                    parts1[2] == parts2[2]):  # Same usecase
...
def analyze_module_distribution(codes: Set[str]) -> Dict[str, Any]:
    ...
    for module in module_stats:
        module_stats[module]["usecases"] = sorted(list(module_stats[module]["usecases"]))
        module_stats[module]["statuses"] = sorted(list(module_stats[module]["statuses"]))
    ...
    for module in module_patterns:
        ...
        for usecase, status in standard_patterns:
            if (usecase, status) not in existing_patterns:
                ...
                has_similar = any(
                    existing_usecase == usecase for existing_usecase, _ in existing_patterns
                )
                if not has_similar and len(existing_patterns) > 2:  # Only for modules with some codes
                    suggestions.append({
...
def check_cross_module_consistency(codes: Set[str]) -> List[Dict]:
    ...
    total_modules = len(set(code.split('-')[1] for code in codes if len(code.split('-')) >= 2))
    ...
    try:
        ...
        if similar_codes:
            ...
            for similar in similar_codes[:5]:  # Show first 5
                print(f"  {similar['base_code']} similar to: {', '.join(similar['similar_codes'])}")
        ...
        for module, stats in sorted(module_dist.items()):
            print(f"  {module}: {stats['total']} codes ({stats['success']} success, {stats['error']} error)")
        ...
        if similar_codes or violations or consistency_issues:
            print(f"  - Warnings: {len(similar_codes + violations + consistency_issues)} (non-critical)")
