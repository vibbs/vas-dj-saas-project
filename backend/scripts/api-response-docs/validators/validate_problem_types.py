...
def validate_problem_type_structure(problem: Dict[str, Any], slug: str) -> List[str]:
    ...
    missing_recommended = [field for field in recommended_fields if field not in problem]
    if missing_recommended:
        errors.append(f"Problem type '{slug}' missing recommended fields: {', '.join(missing_recommended)}")
    ...
    for slug, problem in problem_types.items():
        ...
        try:
            ...
            if not parsed.netloc:
                errors.append(f"Problem type '{slug}' URI '{type_uri}' has invalid domain")
            if not parsed.path:
                errors.append(f"Problem type '{slug}' URI '{type_uri}' has invalid path")
...
def validate_status_codes(problem_types: Dict[str, Dict[str, Any]]) -> List[str]:
    ...
    for slug, problem in problem_types.items():
        ...
        if not isinstance(status, int):
            errors.append(f"Problem type '{slug}' default_status must be an integer, got {type(status).__name__}")
            continue
            
        # Check range
        if not (100 <= status <= 599):
            errors.append(f"Problem type '{slug}' has invalid default_status: {status} (must be 100-599)")
        ...
        if status not in common_statuses:
            errors.append(f"Problem type '{slug}' uses uncommon status code {status} - verify this is intentional")
...
def validate_i18n_keys(problem_types: Dict[str, Dict[str, Any]]) -> List[str]:
    ...
    for slug, problem in problem_types.items():
        ...
        if not i18n_key.replace("_", "").replace(".", "").isalnum():
            errors.append(f"Problem type '{slug}' i18n_key '{i18n_key}' contains invalid characters")
            
        # Recommend format
        if not ("." in i18n_key and i18n_key.startswith(("error", "errors", "problem"))):
            errors.append(f"Problem type '{slug}' i18n_key '{i18n_key}' should follow pattern 'errors.category.type'")

    return errors


def validate_associated_codes(problem_types: Dict[str, Dict[str, Any]], all_codes: Set[str]) -> List[str]:
    ...
    for slug, problem in problem_types.items():
        ...
        for code in codes:
            ...
            if not BaseAPICodeMixin.validate_code(code):
                errors.append(f"Problem type '{slug}' contains invalid code format: {code}")
                continue
...
def check_problem_type_coverage(problem_types: Dict[str, Dict[str, Any]], all_codes: Set[str]) -> List[str]:
    ...
    error_codes = {code for code in all_codes if not code.endswith(('-200', '-201', '-202', '-204'))}
    unreferenced_errors = error_codes - referenced_codes
    
    if unreferenced_errors:
        warnings.append(f"Found {len(unreferenced_errors)} error codes not associated with problem types:")
...
def main():
    ...
    try:
        ...
        print(f"  - Status codes used: {sorted(analysis['status_distribution'].keys())}")
